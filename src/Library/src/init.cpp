...
void LibraryManager::initUtilityFunctions(){
    ...
    }
    {
        ...
        std::string func_code = "std::vector<std::pair<char, int>> runLengthEncoding(std::string s){\n    int n = s.length();\n    std::vector<std::pair<char, int>> res;\n    char pre = s[0];\n    int cnt = 1;\n    for(int i = 1;i < n; i++){\n        if (pre != s[i]){\n            res.push_back({ pre, cnt });\n            pre = s[i];\n            cnt = 1;\n        }\n        else{\n            cnt++;\n        }\n    }\n    res.push_back({ pre, cnt });\n    return res;\n}\n";
        std::string pos = HEAD;
    ...
    }
    {
        ...
        std::string func_code = "template<class T> long long inversionNumber(std::vector<T> &x, std::vector<T> &y){\n    int sz = x.size();\n    assert(sz == (int)y.size());\n    Fenwick_tree<long long> bit(sz);\n    std::map<T, std::vector<int>> idx;\n    for(int i=sz-1;i>=0;i--){\n        idx[x[i]].emplace_back(i);\n    }\n    long long res = 0;\n    for(int i=0;i<sz;i++){\n        std::vector<int> &v = idx[y[i]];\n        if(v.empty()){\n            return -1;\n        }\n        int id = v.back();\n        v.pop_back();\n        res += id - bit.sum(id);\n        bit.add(id, 1);\n    }\n    return res;\n}\n";
        std::string pos = HEAD;
        std::vector<std::string> deps;
